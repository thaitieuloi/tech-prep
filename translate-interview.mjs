...
function run(t) {
  t.forEach(x => {
    ...
    if (p.length > 1) {
      ...
      console.log("unmatched: ", x.en.substring(0,20));
      let safeStr = x.en.replace(/&lt;/g, '<').replace(/&gt;/g, '>');
      p = html.split(safeStr);
      if (p.length > 1) {
        html = p.join('<span class="en-hide">' + safeStr + '</span><div class="lang-dual"><span class="lang-en">' + safeStr + '</span><span class="lang-vi pt-2 block mt-2 text-indigo-400 font-semibold" style="border-top:1px dotted rgba(255,255,255,0.15)">' + x.vi + '</span></div>');
        console.log(" matched safe: " + safeStr.substring(0,20));
      }
    }
  });
}
