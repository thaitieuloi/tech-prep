...
function run(t) {
  t.forEach(x => {
    ...
    if (p.length > 1) {
      ...
      html = p.join('<span class="en-hide">' + x.en + '</span><div class="lang-dual"><span class="lang-en">' + x.en + '</span><span class="lang-vi pt-2 block mt-2 font-semibold" style="color: #6366f1; border-top:1px dotted rgba(255,255,255,0.15)">' + x.vi + '</span></div>');
    }
  });
}
run(t2);
